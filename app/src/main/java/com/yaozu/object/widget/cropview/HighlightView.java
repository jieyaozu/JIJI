...
package com.yaozu.object.widget.cropview;
...
public class HighlightView {
    ...
    public float getWidthCap() {
        return widthCap;
    }

    public void setWidthCap(float widthCap) {
...
//                    mResizeDrawableDiagonal.draw(canvas);
    ...
    public int getHit(float x, float y) {
        ...
        if (mCircle) {
            ...
            if ((Math.abs(r.left - x) < hysteresis) && verticalCheck) {
                retval |= GROW_LEFT_EDGE;
            } else if ((Math.abs(r.right - x) < hysteresis) && verticalCheck) {
                retval |= GROW_RIGHT_EDGE;
            } else if ((Math.abs(r.top - y) < hysteresis) && horizCheck) {
                retval |= GROW_TOP_EDGE;
            } else if ((Math.abs(r.bottom - y) < hysteresis) && horizCheck) {
                retval |= GROW_BOTTOM_EDGE;
            }
    ...
    public void handleMotion(int edge, float dx, float dy) {
        ...
        if (edge == GROW_NONE) {
            ...
            growBy(edge, (((edge & GROW_LEFT_EDGE) != 0) ? -1 : 1) * xDelta, (((edge & GROW_TOP_EDGE) != 0) ? -1 : 1) * yDelta);
        }
    }
    ...
    void moveBy(float dx, float dy) {
        ...
        //System.out.println(mCropRect.left+"-"+mCropRect.top+"-"+mCropRect.right+"-"+mCropRect.bottom);
